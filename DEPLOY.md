...
## Réglage Cloudflare important

Dans Cloudflare, la commande de déploiement peut rester :

```bash
npx wrangler deploy
```

Le fichier `wrangler.toml` contient maintenant une section `[build]` qui lance automatiquement :

```bash
bun run build
```

avant le déploiement. Cela crée le fichier `.output/server/index.mjs` demandé par Wrangler.

Commande possible aussi :
